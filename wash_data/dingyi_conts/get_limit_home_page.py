...
with balalaba.cursor() as cursor:
    ...
    cursor.execute('''select distinct home_page from comps where createdAt > "2019-01-14" and createdAt < "2019-01-20" and length(home_page) > 1;''')
    res = cursor.fetchall()
...
def main(res_, i):
    for url in res_:
        if url.get('home_page'):
            with open(r'C:\Users\YQ\Desktop\dingyi_contacts\comp_2019-01-23\comp{}_2019_01_23.txt'.format(i), 'a+', encoding='utf-8') as file:
