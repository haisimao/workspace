...
def refer_data(table, frequency):
    with balalaba.cursor() as cursor_balalaba:
        # cursor_balalaba.execute('''select * from {} where createdAt < '2019-01-29' and createdAt > '2018-12-24 15:04:00'; '''.format(table))
        cursor_balalaba.execute('''select * from {} where createdAt > '2019-02-20' and length(cont_email) > 0  and createdAt < '2019-02-26'; '''.format(table))
    ...
    print(data.shape)
    num = len(data)
    page = 100000 if table == 'comps' else 100000
    print('插入', data.shape)
